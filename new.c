...
float angleX = 0.0f, angleY = 0.0f; // View rotation from mouse drag
...
{ 
	...
	glLoadIdentity();
	glTranslatef(0.0, 0.0, -10.0); // Move camera back
	glRotatef(angleX, 1.0, 0.0, 0.0); // Rotate around X-axis (vertical drag)
	glRotatef(angleY, 0.0, 1.0, 0.0); // Rotate around Y-axis (horizontal drag)
		glColor3fv(color[0]); 
	output(1,8,message);
	glColor3f(0.0,0.0,0.1);
	output(-9,8,"Chuda");
	output(-9,7,"1RVU23LCSE003");
	glColor3fv(color[0]); 
...
void spincube() 
{
	...
	angleX = 20.0f;  // Or your desired starting view angles
angleY = -30.0f;
