...
char defaultMessage[] = "Rubics Cube Rotation: "; 
...
static GLint axis=0; 
//the rotation parameters for the 3D space.
...
void colorcube2() 
{ 
	polygon(6,8,11,10,9);  						//(front face of unit cube)
	polygon(6,10,11,15,14);  					//(right face of unit cube)
	polygon(6,8,12,15,11);    // bottom center   //(top face of unit cube)
	polygon(6,9,10,14,13);            			//(left face of unit cube)
	polygon(6,12,13,14,15); 					//(back face of unit cube)
	polygon(bottom[1][1],8,9,13,12);  			//(bottom face of unit cube)
} 
void colorcube3() 
{ 
...
{ 
	...
	//Rotates the object by 25.0 + p degrees around the X-axis.
	...
	//rotation == 1 
	//used to rotate the top face of the cube.
	if(rotation==1) 
	{ 
		colorcube1(); 
		colorcube2(); 
	...
	//used to rotate the right face of the cube.
	if(rotation==2) 
	{  
		colorcube1(); 
		colorcube2(); 
	...
	//	//used to rotate the front face of the cube.
	if(rotation==3) 
	{ 
		colorcube1(); 
		colorcube2(); 
	...
	//used to rotate the left face of the cube.
	if(rotation==4) 
	{  
		colorcube1(); 
		colorcube2(); 
	...
	//used to rotate the back face of the cube.
	if(rotation==5) 
	{  
		colorcube1(); 
		colorcube2(); 
		...
		//used to rotate the bottom face of the cube.
	if(rotation==6) 
	{  
		colorcube1(); 
		colorcube3(); 
	...
	//updated scene after applying the transformations.
	glFlush(); 
	glutSwapBuffers(); 
} 
...
{  
	...
	{ 
		...
		//top-left corner 
		temp=right[0][0]; 
		//Moves the bottom-left corner to the top-left corner of the right matrix.
		right[0][0]=right[2][0]; 
		//Moves the bottom-right corner to the bottom-left corner of the right matrix.
		right[2][0]=right[2][2]; 
		//Moves the top-right corner to the bottom-right corner of the right matrix.
		right[2][2]=right[0][2]; 


		//Places the original top-left corner (stored in temp) into the top-right corner 
		//of the right matrix, completing the corner rotation.
		right[0][2]=temp; 
	
		//Stores the middle-left element of the right matrix.
		temp=right[1][0]; 
		//Moves the bottom-center element to the middle-left element of the right matrix.
		right[1][0]=right[2][1]; 
		//Moves the top-center element to the bottom-center element of the right matrix.
		right[2][1]=right[1][2]; 
		//Moves the top-center element to the middle-right element of the right matrix.
		right[1][2]=right[0][1]; 

		//Places the original middle-left element (stored in temp) 
		//into the top-center element of the right matrix, completing the edge rotation.
		right[0][1]=temp; 
	} 
	if(a=='t') 
	{ 
...
//Angle Update //Rotation Completion Check 
//Face Rotation Based on Input //Reset Rotation Values:
//Redisplay
void spincube() 
{
	theta+=0.5+speed; 
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        case 'a':  // Rotate top clockwise
            rotation = 1; inverse = 0;
            solve[++count] = 1;
            glutIdleFunc(spincube);
            break;
        case 'q':  // Rotate top counter-clockwise
            rotation = 1; inverse = 1;
            solve[++count] = -1;
            glutIdleFunc(spincube);
            break;
        case 's':  // Rotate right clockwise
            rotation = 2; inverse = 0;
            solve[++count] = 2;
            glutIdleFunc(spincube);
            break;
        case 'w':  // Rotate right counter-clockwise
            rotation = 2; inverse = 1;
            solve[++count] = -2;
            glutIdleFunc(spincube);
            break;
        case 'd':  // Rotate front clockwise
            rotation = 3; inverse = 0;
            solve[++count] = 3;
            glutIdleFunc(spincube);
            break;
        case 'e':  // Rotate front counter-clockwise
            rotation = 3; inverse = 1;
            solve[++count] = -3;
            glutIdleFunc(spincube);
            break;
        case 'f':  // Rotate left clockwise
            rotation = 4; inverse = 0;
            solve[++count] = 4;
            glutIdleFunc(spincube);
            break;
        case 'r':  // Rotate left counter-clockwise
            rotation = 4; inverse = 1;
            solve[++count] = -4;
            glutIdleFunc(spincube);
            break;
        case 'g':  // Rotate back clockwise
            rotation = 5; inverse = 0;
            solve[++count] = 5;
            glutIdleFunc(spincube);
            break;
        case 't':  // Rotate back counter-clockwise
            rotation = 5; inverse = 1;
            solve[++count] = -5;
            glutIdleFunc(spincube);
            break;
        case 'h':  // Rotate bottom clockwise
            rotation = 6; inverse = 0;
            solve[++count] = 6;
            glutIdleFunc(spincube);
            break;
        case 'y':  // Rotate bottom counter-clockwise
            rotation = 6; inverse = 1;
            solve[++count] = -6;
            glutIdleFunc(spincube);
            break;
...
{ 
	...
	//Enables depth testing to handle 3D rendering correctly.
